...
export default async function getSettings() {
    ...
    for (const key in SETTINGS.keys) {
        const { value } = await Preferences.get({ key: SETTINGS.keys[key as keyof SettingsData] });

        if (!value) {
            settings[key as keyof SettingsData] = SETTINGS.defaults[key as keyof SettingsData] as SettingsData[keyof SettingsData];
